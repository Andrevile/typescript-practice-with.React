...
const reducer = (state: ReducerState, action: ReducerActions): ReducerState => {
  //이전의 스테이트를 action으로 새로운 state를 반환
  switch (action.type) {
    case SET_WINNER:
      return {
        ...state,
...
function TicTacToe() {
  const [state, dispatch] = useReducer<React.Reducer<ReducerState, ReducerActions>>(reducer, initialState); //useReducer가 타이핑 이 안된다면 제네릭
  ...
  useEffect(() => {
    ...
    if (tableData[0][2] === turn && tableData[1][1] === turn && tableData[2][2] === turn) {
      win = true;
    }
